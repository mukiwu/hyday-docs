...
類型需要什麼計費方式支援工具呼叫雲端 API 服務各家供應商的 API Key依 API 用量計價取決於模型CLI 訂閱本機已安裝並登入Claude Code / Gemini CLI / Codex CLI沿用既有訂閱方案，**不另外計費**沿用 CLI 內建工具(含網路搜尋)本機模型本機已安裝並啟動 Ollama完全免費 / 離線視模型而定，多數較弱
