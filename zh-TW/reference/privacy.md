...
此外，行銷網站 hyday.tw 使用 Google Analytics 4 進行**匿名**流量分析（僅頁面瀏覽與下載點擊的彙總數據，並已啟用 IP 匿名化）。流量來源資訊僅記錄標準 UTM 欄位與來源網站的**網域**（絕不記錄完整網址）。此分析僅存在於網站、與桌面應用程式完全分離——**桌面 App 內不含任何分析或追蹤程式碼**
