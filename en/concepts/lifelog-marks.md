...
SyntaxMark NamePurpose`>(HH:mm)`Task start (taskStart)Records**starting something at this time**.`<(HH:mm)`Task end (taskEnd)Records**finishing or stopping something at this time**.`-(HH:mm)`Current (current)Records**what's happening or how you feel right now**.
...
SyntaxMark NamePurpose`%(content)`Thought (thought)Captures a sudden idea, a reflection, or a meta-thought.`!(content)`Important (important)Flags the must-remember points so they're easy to find later.
