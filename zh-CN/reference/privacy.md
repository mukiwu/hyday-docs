...
此外，营销网站 hyday.tw 使用 Google Analytics 4 进行**匿名**流量分析（仅页面浏览与下载点击的汇总数据，并已启用 IP 匿名化）。流量来源信息仅记录标准 UTM 字段与来源网站的**网域**（绝不记录完整网址）。此分析仅存在于网站、与桌面应用程序完全分离——**桌面 App 内不含任何分析或追踪程序代码**
