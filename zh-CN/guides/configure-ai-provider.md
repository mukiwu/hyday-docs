...
类型需要什么计费方式支援工具呼叫云端 API 服务各家供应商的 API Key依 API 用量计价取决于模型CLI 订阅本机已安装并登录Claude Code / Gemini CLI / Codex CLI沿用既有订阅方案，**不另外计费**沿用 CLI 内建工具(含网络搜索)本机模型本机已安装并启动 Ollama完全免费 / 离线视模型而定，多数较弱
