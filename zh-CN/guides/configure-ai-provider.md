...
---
> * Hyday 支援三种接入 AI 的方式：**云端 API 服务**、**CLI 订阅** (Claude Code / Antigravity / Codex CLI)、以及**本机模型** (Ollama)。所有设置都在**设置 → AI** 页面中管理 *

## 三种接入方式的差异

   类型 需要什么 计费方式 支援工具呼叫     云端 API 服务 各家供应商的 API Key 依 API 用量计价 取决于模型   CLI 订阅 本机已安装并登录Claude Code / Antigravity / Codex CLI 沿用既有订阅方案，**不另外计费** 沿用 CLI 内建工具(含网络搜索)   本机模型 本机已安装并启动 Ollama 完全免费 / 离线 视模型而定，多数较弱   
...
- **Antigravity**：依照 [Antigravity 官方文件](https://antigravity.google/product/antigravity-cli) 安装 `Antigravity CLI` 并完成登录
...
> **ℹ️ Info**
>  如果已经有 Claude / ChatGPT 订阅，**CLI 订阅是最划算的选择**，不会额外计费还能用到较好的模型；若没有订阅、想随用随付，云端 API 较简单；对隐私要求极高就选 Ollama 
